...
export async function insertCourseData(
  ...
  try {
    await db.execute(`
    BEGIN TRANSACTION;
      INSERT INTO courses (department, courseNumber, title, description, requirements, units, minLevel, fall, winter, spring)
      VALUES ('${transformedCourse.course.department}', '${transformedCourse.course.courseNumber}', '${transformedCourse.course.title}', ${transformedCourse.course.description ? `'${transformedCourse.course.description}'` : "NULL"}, ${transformedCourse.course.requirements ? `'${transformedCourse.course.requirements}'` : "NULL"}, ${transformedCourse.course.units ?? "NULL"}, ${transformedCourse.course.minLevel ? `'${transformedCourse.course.minLevel}'` : "NULL"}, ${transformedCourse.course.fall}, ${transformedCourse.course.winter}, ${transformedCourse.course.spring})
      ON CONFLICT (department, courseNumber) DO UPDATE SET
